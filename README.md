...
![Version](https://img.shields.io/badge/版本-1.0.7-blue?style=flat-square)
...
### v1.0.7 (2025-12-07)
- **新功能**：
  - **访问统计系统**：记录文章阅读量和每日访客数据。
  - **管理后台流量图表**：柱状图 + 累计趋势曲线，直观展示近 7 天访问情况。
  - **AI 智能摘要生成**：编辑文章时可一键生成 50-150 字摘要。
- **权限优化**：
  - 普通用户可在管理后台发布、编辑、删除**自己的文章**。
  - 管理员可管理**所有用户的文章**。
  - 用户个人主页新增文章编辑/删除按钮。
- **Bug 修复**：
  - 修复 Vite 代理导致的 `X-Forwarded-For` 验证错误。
